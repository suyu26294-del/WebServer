...
这是一个从零实现的高性能 Linux C++ WebServer，面向 C++/服务端岗位面试与企业级工程实践。
...
| 当前实现 | 企业级系统 |
|----------|-----------|
| 单 Reactor | 主从 Reactor（多个 EventLoop） |
| 简单日志系统 | 分布式链路追踪 |
| 基础 HTTP/1.1 | HTTP/2、gRPC |
| 无限流 | 限流、熔断、降级 |
| 无监控 | Prometheus 指标、告警 |
| 单机 | 集群、服务发现 |
...
epoll 使用红黑树管理 fd，事件就绪时只返回活跃 fd（O(活跃数)），而 select/poll 每次需要遍历全部 fd（O(总数)）。此外 epoll 通过 mmap 避免用户态/内核态数据拷贝。

**Q: ET 和 LT 的区别？**

LT（水平触发）：只要 fd 可读就持续通知，可不一次读完。ET（边缘触发）：fd 状态变化时通知一次，必须循环读到 EAGAIN，否则可能丢失数据。ET 通知次数少，性能更高，但编程复杂度更高。

**Q: EPOLLONESHOT 的作用？**

注册后只触发一次，触发后自动从 epoll 摘除。保证同一 fd 同一时刻只被一个工作线程处理，避免竞态。处理完毕后需手动 `epoll_ctl(MOD)` 重新注册。

**Q: 为什么要有定时器？**

HTTP keep-alive 连接可能长期空闲不关闭，造成 fd 和内存泄漏，epoll 里也会积累大量无效监听项。定时器定期检查并关闭超时连接，保证服务稳定运行。

---

## 作者

`CppWebServer` — 面向 C++/服务端岗位的工程实践项目
